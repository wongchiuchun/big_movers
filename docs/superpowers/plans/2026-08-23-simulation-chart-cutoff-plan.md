...
- [x] **Step 1: Add one revealed-bar helper**
...
- [x] **Step 2: Route chart producers through the helper**
...
- [x] **Step 3: Stop timeframe buttons from reloading the ticker during Sim**

In `setTimeframe`, call `Sim.Ctrl.refreshChartCutoff()` when `Sim.Ctrl.isActive()` instead of `selectRow(activeIdx)`. Keep the existing reload path outside Sim.
...
- [x] **Step 1: Resample only revealed daily bars**
...
- [x] **Step 2: Update partial weekly/monthly candles safely**
...
- [x] **Step 3: Keep moving averages causal across timeframe changes**
...
- [x] **Step 4: Preserve simulation overlays after a timeframe rebuild**

Expose a small `Sim.Ctrl.refreshChartCutoff()` method that obtains the inclusive cutoff through `getCutoffBarTime()`, resolves the corresponding revealed slice, and recomputes/remasks candles, volume, moving averages, SuperTrend, AI overlays, drawings, and current simulation markers. Call it instead of the ordinary ticker reload/render while Sim is active. Blind/Random Sim uses the same controller and cutoff contract.
...
- [x] **Step 1: Review focused source paths**
...
- [x] **Step 3: Commit only feature files**

...
## Annotation note discipline

**Never trust `reviews.json` for EP/gap/breakout dates when writing note text.** The `snapshot.big_gaps`, `legs`, and `rationale` fields were generated by AI visual inspection (`annotation_type: "AI independent attempt"` or `"… (REDO with visual inspection)"`) and contain hallucinated dates and stats. Treat those fields as prose, not data.

**Past failure:** CLS note id 67 said *"Aug 5 EP: +9% gap on 2.5x vol"*. Real EP was **Jul 29** (+8% gap on 3.8x vol, close@74%). The text was copied verbatim from `reviews.json` → `CLS_2025.snapshot.big_gaps[1]`.

**Source of truth for dates and stats:**

1. **`tools/analyze_move.py SYMBOL [YEAR]`** — computes the gap list deterministically from the raw OHLCV CSV (filter: gap≥5% on vol≥1.5× 20d). **Always run this before writing any date into note text.**
2. **`ai_classifications.json`** — `[TICKER_YEAR].pivot.pivot_date` and `ai_classifications[*].extra.gap_date` are also deterministic (produced by `classifier/pipeline.py`).

**Before committing note edits, run the validator:**

```bash
/Library/Frameworks/Python.framework/Versions/3.13/bin/python3 tools/check_note_dates.py
```

It flags single-event EP/gap notes whose text date drifts >3 days from the note's `p1` anchor — the class of bug that produced the CLS Aug-5/Jul-29 mismatch.

## Annotation note voice

Separate from date accuracy: the *text* inside each note must read like a trader reviewing their trade decisions, not like a quant labeling outcomes. The user's [Human] notes consistently beat AI-drafted notes along 7 axes. Full before/after examples and the vocabulary table live in `.claude/commands/annotate.md` Step 9b ("Note voice rules"). **v1 — user said "still see gaps here" on 2026-04-15; expand when new `[Human]` reviews land in `metadata.json`.**

Quick checklist before writing any note text:

1. **Name the pattern, not the stat.** Use "U&R", "test of faith", "character change", "tight EMA action", "climax top" — not "49% 10EMA, 10 50SMA touches".
2. **Write the *reason* to enter.** "Prior day tested 200EMA and held" > "base breakout". The decision inputs beat the entry label.
3. **Chain phantom trades.** "First exit stop out" → "U&R play failed" → "Would have been stopped out but U&R" → "D+2 held LoD". Tradability is about what survives a sequence.
4. **Own in-the-moment ambiguity.** "Looked like a climax top but the retest held 10MA. Should have exited 1/3." Never retrospectively confident.
5. **Split by character change.** Not Q1/Q2/Q3/Q4 time splits. Segment when personality shifts.
6. **Consider the short side** once the long side breaks down. Every parabolic → breakdown has a shortable leg worth noting.
7. **Continuous difficulty scale**, not TRADABLE/PARTIAL/NOT buckets. "Quite tradable", "hard but possible", "not my favorite target".

Before saving to drawings.json, audit each note against the 6 self-review questions at the bottom of Step 9b.
